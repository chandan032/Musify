...
def play_song_name(request, name):
    songs = Song.objects.filter(name__icontains=name).first()
    if not songs:
         songs = Song.objects.filter(id=1).first()
...
def last_played(request):
    ...
    last_played_list = list(Recent.objects.values('song_id').order_by('-id'))
    if last_played_list:
        last_played_id = last_played_list[1]['song_id']
        last_played_song = Song.objects.get(id=last_played_id)
    else:
        last_played_song = Song.objects.get(id=7)
